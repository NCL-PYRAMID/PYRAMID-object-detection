...
class DetectorModel():
    ...
    def inference_single_vis(self, srcpath, dstpath, slide_size, chip_size):
        ...
        with open(os.path.join(dstpath[:-4], "classnames.txt"), "w") as op:
...
#     current_path = os.getcwd()
    ...
    for imgnames in os.walk(os.path.join(current_path, 'test_imagery_ts/'), topdown=False):
        for i, img in enumerate(list(imgnames[2])):
            img_id = os.path.join(current_path, 'test_imagery_ts/', img)
